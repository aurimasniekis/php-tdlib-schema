...
class MessageReplyInfo extends TdObject
{
    ...
    public function __construct(int $replyCount, array $recentRepliers, int $lastReadInboxMessageId, int $lastReadOutboxMessageId, int $lastMessageId)
    {
    ...
    public static function fromArray(array $array): MessageReplyInfo
    {
        return new static(
            $array['reply_count'],
            array_map(fn ($x) => TdSchemaRegistry::fromArray($x), $array['recentRepliers']),
